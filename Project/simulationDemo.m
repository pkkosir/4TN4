function simulationDemo(model, wordClasses)
    ...
    if nargin < 1 || isempty(model)
        if exist('trained_model.mat', 'file')
            ...
            % Load into a structure to avoid workspace conflicts
            data = load('trained_model.mat');
            model = data.trainedModel;
            
            % Check if wordClasses is also available in the loaded data
            if nargin < 2 || isempty(wordClasses)
                if isfield(data, 'wordClasses')
                    wordClasses = data.wordClasses;
                end
            end
        else
    ...
    % Default word classes if not provided
    if nargin < 2 || isempty(wordClasses)
    ...
    if exist('lip_features.mat', 'file')
        ...
        % Load into a structure to avoid workspace conflicts
        featData = load('lip_features.mat');
        
        % Extract validation set (20% of data or at least 1 sample per class)
        [testFeatures, testLabels] = extractValidationSet(featData.features, featData.labels, 0.2);
    else
